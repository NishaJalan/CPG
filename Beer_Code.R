...
setwd('C:/Users/jalan/Desktop/IRI')
...
colnames(beer.total)[colnames(beer.total)=="COLUPC"]<-'Vend_Item'
...
beer.total.order<-beer.total[,c(8,1,5,2,7,4,6,3)]
colnames(beer.total.order)[colnames(beer.total.order)=="OUTLET"]<-'ChannelType_ID'


#Write the final beer.total file
setwd('C:/Users/jalan/Desktop/IRI/New Files/Recheck/21Nov')
write.csv(beer.total, file = paste('beer.total.csv',sep = '\t'), row.names = T)
write.csv(beer.total.order, file = paste('beer.total.order.csv',sep = '\t'), row.names = T)
#############
...
library(readxl)
parsed_beer <-read_excel("C:/Users/jalan/Desktop/IRI/New Files/Recheck/prod12_beer.xlsx")
...
parsed_beer<-parsed_beer[,-c(1,3,4,6,7,8,9,10,11,12,13,15,18,19,20,21)]
...
head(Merged.data)
# beer.total.final<-Merged.data[,c(1,2,6,3,8,5,7,4)]
# beer.total.final<-beer.total.final[!duplicated(beer.total.final),]

#write.csv(Data1, file = paste('Data1.csv',sep = '\t'), row.names = T)
parsed_beer_filtered<-Merged.data[,-c(2:8)]
parsed_beer_filtered<-parsed_beer_filtered[!duplicated(parsed_beer_filtered),]

#Crearte product type variable from L2=Merchandise Type 
...
table(parsed_beer_filtered$Merchandise_Type)
levels(parsed_beer_filtered$Merchandise_Type)<-c(1,2,3,1)
...
parsed_beer_filtered.order<-parsed_beer_filtered[,c(1,6,7,2,3,5,4)]

#Writing the final files
setwd('C:/Users/jalan/Desktop/IRI/New Files/Recheck/21Nov')
write.csv(parsed_beer_filtered, file = paste('parsed_beer_filtered.csv',sep = '\t'), row.names = T)
write.csv(parsed_beer_filtered.order,file=paste('parsed_beer_filtered.order.csv',sep='\t'),row.names=T)
